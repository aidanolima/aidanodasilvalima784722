...
import { toast } from 'react-toastify'; // Importação adicionada para UX
  ...
  const loadStats = async () => {
    try {
      ...
      const vinculadosCount = tutorsList.reduce((acc: number, tutor: any) => {
        // Checagem segura: garante que estamos tentando ler .length de algo que existe
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 p-4 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <StatCard title="Total de Pets" value={stats.pets} icon={Dog} colorClass="text-blue-600" bgClass="bg-blue-50" borderColor="border-blue-100" />
        <StatCard title="Tutores Cadastrados" value={stats.tutors} icon={Users} colorClass="text-emerald-600" bgClass="bg-emerald-50" borderColor="border-emerald-100" />
        <StatCard title="Pets Vinculados" value={stats.linked} icon={HeartHandshake} colorClass="text-purple-600" bgClass="bg-purple-50" borderColor="border-purple-200 border-b-4 border-b-purple-600" />
      </div>
